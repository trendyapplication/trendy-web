...
//        print_r($array);
        ...
        foreach ($return as $key => $ret) {
            ...
            if (sizeof($sec_array) > 0) {
                ...
                if ($cou3 > 0) {
                    ...
                    $super_parent_array[$i]['parent_array'] ['type_array'] = array();
                    $i++;
                }
            }
    ...
    function get_all_type_trend($city, $gender) {
        ...
        echo '<pre>';
        foreach ($super_parent_array as $super_arr) {
            print_r($super_arr);
        }
    }
...
//            $ret_array[] = $social;
    ...
    function get_brand_full($city) {
        $data = $this->db->query("SELECT * FROM (select T1.*,(SELECT count(*) AS num  FROM trend_post WHERE brand=T1.id and posted_locatn='$city' ) AS count_num 
...
//        echo $data;
    ...
    function pop_price_status($lat, $long, $city) {
        $price_max = $this->db->query("select price from trend_post where posted_locatn='$city' order by price desc limit 1");
        $primax = $price_max->row_array();
        $return['max_price'] = $primax['price'];
        $price_max1 = $this->db->query("select price from trend_post where posted_locatn='$city' order by price asc limit 1");
...
FROM (SELECT id as id1 FROM trend_post WHERE posted_locatn='$city') as T1 LEFT JOIN trend_vote T2 ON T1.id1=T2.post_id GROUP BY T2.post_id ORDER BY maxCountss asc limit 1");
        $popMin = $pop_mi->row_array();
        if ($popMin['maxCountss'] > -5) {
...
FROM (SELECT id as id1 FROM trend_post WHERE posted_locatn='$city') as T1 LEFT JOIN trend_vote T2 ON T1.id1=T2.post_id GROUP BY T2.post_id ORDER BY maxCountss desc limit 1");

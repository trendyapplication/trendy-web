...
//						$update_id = $this->user_model->update_by(array('member_id'=>$user_id), array('status'=>'T'));				
    ...
    function uploadImage() {
        if ($_FILES['ocimage']['name'] != '') {
            ...
            $new_file_name = $_POST['id'] . '.' . $ext[1];
            echo $new_file_name . "?" . rand(111, 900000000000);
            unlink(base_url() . 'uploads/members/' . $_POST['id'] . $ext[1]);
            move_uploaded_file($_FILES['ocimage']['tmp_name'], 'uploads/members/' . $new_file_name);
            $this->gallery_path = realpath(APPPATH . '../uploads'); //fetching path
            $config1 = array(
                'source_image' => $this->gallery_path . '/members/' . $new_file_name, //get original image
                'new_image' => $this->gallery_path . '/thumbs/members/' . $new_file_name, //save as new image //need to create thumbs first
                'maintain_ratio' => true,
                'width' => 165,
                'height' => 120
            );
            $arr1 = array("img_extension" => $ext[1]);
            $cond = array("user_id" => $_POST['id']);
            $this->user_model->update_user_ext($arr1, $cond);
            $this->load->library('image_lib', $config1); //load library
            $this->image_lib->resize();
            $targetFile = realpath('uploads/members') . "/" . $new_file_name;

...
//        print_r($data);
    ...
    function edit_statedetails($id) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (!$id) {
                 $url = "http://maps.google.com/maps/api/geocode/json?address=" . urlencode($_POST['city_name']) . "," . urlencode($country_name_id['name']) . "&sensor=false&region=India";
//  echo $url;
//  exit;
                $ch = curl_init();
                ...
                $array = array("city" => $_POST['city_name'],"lat"=>$lat,"long"=>$long, "state" => $_POST['state'], "country" => $_POST['country_name']);

                $this->country_model->insert_state($array);
            } else {
                $country_name_id = $this->country_model->get_brand_details($_POST['country_name']);
...
//  exit;
                ...
                $cond_post = array("posted_locatn_id" => $id);
                $test = $this->country_model->update_state_post($array_post, $cond_post);
...
//                exit;
            ...
            $data['get_state'] = $this->country_model->getstate_gnLists();

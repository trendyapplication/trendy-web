...
//echo "df";
    ...
    function delete($id){
        $array=array("id"=>$id);
        $this->user_model->del_report($array);
...
//        exit;
        ...
        $config['total_rows'] = $this->user_model->getReportCount($data['status'], $data['key']);
        $data['userlist'] = $this->user_model->getReportList($data['status'], $config['per_page'], $_REQUEST['per_page'], $data['key']);
        ...
        $config['base_url'] = site_url("admin/report/lists") . "/" . $params;
        ...
        $output['output'] = $this->load->view('admin/report/lists', $data, true);

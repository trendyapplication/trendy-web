...
class Client extends CI_Controller {
    ...
    public function index() {
        ...
        //        $json = '{"function":"forgot_password","parameters": {"v": "1.0","apv": "I-1.0","authKey": "9c833a407cbbf0e2ab7c9ca9fca744a3","sessionKey": "7ebsQ3mY3C1lNklt7Zho","email": "aneeshg@newagesmb.com"},"token":""}';
//        $json='{"function":"send_activation","parameters": {"v": "(null)","apv": "(null)","authKey": "(null)","sessionKey": "(null)","user_id":218},"token":""}';
//        $json = '{"function":"registration","parameters": {"v": "(null)","apv": "(null)","authKey": "(null)","sessionKey": "(null)","name": "Biju George","user_name": "aneeshg","email": "aneeshg@newagesmb.com","password": "123456","gender": "Male","device_id": "4bac17994d549adb50f2f7933da2d660676ff3bd3d3782128bd4ce2749fb0318"},"token":""}';
...
//$json='{"function":"uptest","parameters": {"v": "1.0","apv": "I-1.0","authKey": "9c833a407cbbf0e2ab7c9ca9fca744a3","sessionKey": "Q0C0en5nf4rO0Y9YTAfW","user_id": "137","lat": "10.074581","long": "76.278854","occasion_id": "60"},"token":""}';
...
//        $json=' {"function":"fb_registration","parameters": {"v": "1.0","apv": "I-1.0","authKey": "9c833a407cbbf0e2ab7c9ca9fca744a3","sessionKey": "M0Ro8zAz6GRuLDOp7UHk","fb_unique_id": "1632176873733383","name": "Anamika Anamika","user_email": "umesh@newagesmb.com","user_gender": "male","fburl": "http://graph.facebook.com/1632176873733383/picture??width=320&height=320","device_id": "4bac17994d549adb50f2f7933da2d660676ff3bd3d3782128bd4ce2749fb0318"},"token":""}';
    ...
    function uptest() {
        ...
        var_dump(shell_exec("/usr/local/bin/s3cmd  --config=/var/www/html/trend/.s3cfg     --acl-public put " . $targetFile . "    s3://trendyservice/members/"));
//        shell_exec("s3put trendyservice/$image_name $targetFile");
    }
...
//        $foldername1 = "member" . $imagename;
//        $input1 = base_url() . 'uploads/members/' . $imagename;
//        shell_exec("/usr/local/bin/s3cmd --config=/var/www/html/trendyservice/.s3cfg --acl-public put " . $targetFile . "    s3://trendyservice/members/");
        shell_exec("/usr/local/bin/s3cmd  --config=/var/www/html/trend/.s3cfg     --acl-public put " . $targetFile . "    s3://trendyservice/members/");
        $result = array('status' => 'true', 'width' => $width, 'height' => $height, 'message' => "Profile image updated", 'img_url' => "https://s3.amazonaws.com/trendyservice/members/" . $imagename . "?" . uniqid());
        echo json_encode($result);
    }
...
//                $city = $this->users_model->find_Loc($latti, $longitude);
                $result = array('status' => 'true', 'user' => $userdet, 'userType' => $userdet['userType'], 'active' => 'Y', 'user_id' => $userdet['user_id'], 'fb_unique_id' => $userdet['facebook_id'], 'profile_image' => $userDetails['original_image'], 'password' => $userdet['password'], 'message' => 'Your account is already Connected ');
                echo json_encode($result);
            } else {
    ...
    function Login_user($arr) {
        ...
        $check_login = $this->users_model->check_login($user_name, $password);
        if ($check_login['count'] == 0) {
            $result = array('status' => 'false', 'message' => "Login failed");
        } else {
            if ($check_login['active'] == "Y") {
                $result = array('status' => 'true', 'user' => $check_login, 'message' => "Login completed succesfully");
            } else {
                $result = array('status' => 'true', 'user' => $check_login, 'message' => "This user is blocked by admin ");
            }
        }
        echo json_encode($result);
    }
    ...
    function forgot_password($arr) {
        $email = $arr['email'];
        $user = $this->users_model->get_user_email($email);
        if ($user != 0) {
            $user_id = $user['user_id'];
            $act_array = array("Ball", "Bat", "grey", "Team", "East", "West", "Mushroom", "NorthEast", "WestBengal", "QueenOfArabianSea", "HouseBoat", "NaganjeriMana", "Arsenal", "Munich", "Chelsea", "Barcelona", "Manchaster", "Marriot", "Ayurveda", "Sidha", "Mumbai", "Chenai", "Bengalaru", "Blasters", "Pune", "Delhi", "MegaStar", "LeicesterCity", "Tottenham", "Karnan", "Karma", "Kalki", "Bhiman", "Newyork", "Remember", "Website", "Activation", "Account", "hyperlink", "button", "information", "separate", "custom", "friend", "Parameters", "Icecream", "lollipop", "London", "India", "York", "Cat", "Apple", "Dove", "Jack", "Django", "Avatar", "JACKFRUIT", "LOTUS", "Peacock", "Amazon", "Ganguly", "Riquelme", "Madrid", "SachinTendulkar", "Batman", "Train", "Sites", "Mangorepublic", "Cameroon", "JR10", "Poonjar", "Ghana", "Aluva", "Kakanad", "Padivattom", "BananaRepublic", "Sudan", "Uganda", "Srilanka", "Perumbavoor", "Kothamangalam", "Mangolia", "SHIRTS", "KOCHi");
            $act_key = rand(0, (sizeof($act_array) - 1));
            $new_password = rand(10000, 9999999999999999) . $act_array[$act_key];
            $array = array("password" => md5($new_password));
            $condition = array("user_id" => $user_id);
            $this->users_model->fgt_pwd_update($array, $condition);
            $message = $this->users_model->get_fgt_pwd();
            ...
            $message = str_replace('#FIRST_NAME#', ucfirst($user["name"]), $message);
            ...
            $this->email->subject("New Password For Trendy Service");
            $this->email->message($message);
            $this->email->send();
            $result = array('status' => 'true', 'user_id' => $user_id, 'message' => "New password sent to your mail successfully");
        } else {
            $result = array('status' => 'fail', 'message' => "Invalid mail address");
        }
        echo json_encode($result);
    }
    ...
    function change_password($arr) {
        ...
        $old_pwd = $arr['old_password'];
        if (md5($old_pwd) == $user['password']) {
            $array = array("password" => md5($new_password));
            $condition = array("user_id" => $user_id);
            $this->users_model->fgt_pwd_update($array, $condition);
            $result = array('status' => 'true', 'user_id' => $user_id, 'message' => "Password updated succesfully");
        } else {
            $result = array('status' => 'fail', 'message' => "Invalid current password");
        }
        echo json_encode($result);
    }

    function send_activation1($user_id) {
        $activation_code = $this->users_model->get_User($user_id);
        $message = $this->users_model->Email_varification();
        $config['mailtype'] = 'html';
        $config['charset'] = 'utf-8';
        $config['crlf'] = PHP_EOL;
        $config['newline'] = PHP_EOL;
        ...
        $this->email->initialize($config);
        $message = str_replace("#BASE_URL#", base_url(), $message);
        $message = str_replace('#FIRST_NAME#', ucfirst($activation_code["name"]), $message);
        $message = str_replace('#CODE#', $activation_code["activation_code"], $message);
        $this->email->reply_to('support@trendyservices.com', 'Trendy Services');
        $this->email->from($this->config->item('email_from'));
        $this->email->to($activation_code['user_email']);
        $this->email->subject("Your Trendy Activation Code");
        $this->email->message($message);
        $this->email->send();
        return TRUE;
//        $result = array('status' => 'true', 'user' => $activation_code, 'message' => "Varification code send succesfully");
//        echo json_encode($result);
    }

    function send_activation($arr) {
        $user_id = $arr['user_id'];
        $activation_code = $this->users_model->get_User($user_id);
        $message = $this->users_model->Email_varification();
        $config['mailtype'] = 'html';
        ...
        $this->email->message($message);
        $this->email->send();
        $result = array('status' => 'true', 'user' => $activation_code, 'message' => "Activation code sent successfully");
        echo json_encode($result);
    }

    function verify_activation($arr) {
        $user_id = $arr['user_id'];
        $varification_code = $arr['code'];
        $status = $this->users_model->check_varification($user_id, $varification_code);
        if ($status == 0) {
            $result = array('status' => 'false', 'message' => "Invalid Code Verification Failed");
        } else {
            $user = $this->users_model->get_User($user_id);
            $result = array('status' => 'true', 'user' => $user, 'message' => "Verification succesfully completed");
        }
        echo json_encode($result);
    }
    ...
    function registration($arr) {
        $email = $arr['email'];
        $user_name = $arr['user_name'];
        $check_email = $this->check_user_email($email);
        if ($check_email == 1 || $email == '') {
            $check_user_name = $this->check_user_name($user_name);
            if ($check_user_name == 1 || $user_name == '') {
                ...
                $act_array = array("Ball", "Bat", "grey", "Team", "East", "West", "Mushroom", "NorthEast", "WestBengal", "QueenOfArabianSea", "HouseBoat", "NaganjeriMana", "Arsenal", "Munich", "Chelsea", "Barcelona", "Manchaster", "Marriot", "Ayurveda", "Sidha", "Mumbai", "Chenai", "Bengalaru", "Blasters", "Pune", "Delhi", "MegaStar", "LeicesterCity", "Tottenham", "Karnan", "Karma", "Kalki", "Bhiman", "Newyork", "Remember", "Website", "Activation", "Account", "hyperlink", "button", "information", "separate", "custom", "friend", "Parameters", "Icecream", "lollipop", "London", "India", "York", "Cat", "Apple", "Dove", "Jack", "Django", "Avatar", "JACKFRUIT", "LOTUS", "Peacock", "Amazon", "Ganguly", "Riquelme", "Madrid", "SachinTendulkar", "Batman", "Train", "Sites", "Mangorepublic", "Cameroon", "JR10", "Poonjar", "Ghana", "Aluva", "Kakanad", "Padivattom", "BananaRepublic", "Sudan", "Uganda", "Srilanka", "Perumbavoor", "Kothamangalam", "Mangolia", "SHIRTS", "KOCHi");
                $act_key = rand(0, (sizeof($act_array) - 1));
                $activation_code = rand("10000", "99999") . $act_array[$act_key];
                $array = array("user_type" => 2, "userType" => $user_type, "activation_code" => $activation_code, "username" => $user_name, "user_email" => $email, "password" => $password, "register_date" => date("Y-m-d H:i:s"), "active" => "Y", "active_flag" => $active, "name" => $name, "device_id" => $device_id, "email_verified" => $active, "gender" => $gender);
                $user_id = $this->users_model->save_user($array);
                $arrays = array("user_id" => $user_id, "gender" => $gender, "updated_on" => date("Y-m-d H:i:s"));
                if ($this->users_model->check_setiings_user($user_id) == FALSE) {
                    $this->users_model->save_settings($arrays);
                } else {
                    $cond = array("user_id" => $user_id);
                    $this->users_model->up_settings($arrays, $cond);
                }
                if ($email != '') {
                    $this->send_activation1($user_id);
                }
                $result = array('status' => 'true', 'userType' => $user_type, 'active_flag' => $active, 'user_id' => $user_id, 'message' => "Registration completed successfully");
            } else {
                $result = array('status' => 'false', 'message' => "Username is already in use");
            }
        } else {
            $result = array('status' => 'false', 'message' => "Email is already in use");
        }
        echo json_encode($result);
    }

    function check_user_name($uname) {
        $qry = $this->db->query("select count(*) as count from user_master where username='" . $uname . "' and username !=''");
        $count = $qry->row_array();
        if ($count['count'] > 0) {
            return 0;
        } else {
    ...
    function check_user_email($user_email) {
        $qry = $this->db->query("select count(*) as count from user_master where user_email='" . $user_email . "' and user_email!=''");
        $count = $qry->row_array();
        if ($count['count'] > 0) {
            return 0;
        } else {
    ...
    function postImage($post_id) {
        $image = $this->users_model->getImage($post_id);
        $result = array('status' => 'true', 'image' => $image, 'filePath' => 'https://s3.amazonaws.com/trendyservice/post/');
        echo json_encode($result);
    }
        ...
        if ($_POST['flag'] == "Y") {
            ...
            for ($c = 1; $c <= $count; $c++) {
                if ($_FILES['file' . $c]['name'] <> "") {
                    ...
                    $targetFile1 = realpath("uploads/thumbs/post") . "/" . "thumb_post_640_" . $imagename;
                    shell_exec("/usr/local/bin/s3cmd  --config=/var/www/html/trend/.s3cfg     --acl-public put " . $targetFile1 . "    s3://trendyservice/post/");
                }
            }
        } else {
            $image_url1 = $_POST['img_url'];
//            print_r($_POST['img_url']);
...
//        print_r($image_array);
        ...
        if ($bot['bot'] == "NO") {
            ...
            $city_id = $this->users_model->find_Loc_id($latti1, $longitude1);
        ...
        $insert_array = array("occasion_id" => $occasion_id, "product_type" => $product_type, "product_name" => $product_name, "price" => $price, "brand" => $brand, "gender" => $gender, "description" => $description, "fileNAME" => $fileNAME, "user_id" => $user_id, "lat" => $latti, 'long' => $longitude, 'product_url' => $site_url, 'posted_locatn' => $city, 'posted_locatn_id' => $city_id);
        $insert_id = $this->users_model->new_post($insert_array, $image_array);
        $point = 3;
        $arr = array("user_id" => $user_id, "reward" => $point, "reward_type" => "product_post", "product_id" => $insert_id);
        $array = array("reward_flag" => "Y");
        $cond = array('id' => $insert_id);
...
//        var_dump($ctx);
    ...
    function trendy_trends($arr) {
        ...
        $result = array('status' => 'true', 'message' => $message, 'user_status' => $user_status, 'lat' => $lattitude2, 'longitude' => $longitude2, 'location' => $city, 'recent_posts' => $data, 'vote_status' => $vote_status, 'filePath' => 'https://s3.amazonaws.com/trendyservice/post/');
        echo json_encode($result);
    }
...
//                }
            ...
            $user = $this->users_model->get_settings($user_id);
            $result = array('status' => 'true', 'user' => $user, 'string_length' => $strlen_html, 'product_name' => $brandName, 'image_array' => $image_array, 'brand_name' => $brand, 'image' => $img, 'price' => $price, 'description' => $desc);
        } else {
            $image_array = array();
            $brandName = '';
            $price = '';
            ...
            $user = $this->users_model->get_settings($user_id);
            $desc = '';
            $result = array('status' => 'false', 'user' => $user, 'string_length' => $strlen_html, 'product_name' => $brandName, 'image_array' => $image_array, 'brand_name' => $brandName, 'image' => $img, 'price' => $price, 'description' => $desc);
        }
        echo json_encode($result);
    }
    ...
    function social($arr) {
        ...
        $result = array('status' => 'true', 'notification_unread_count' => $msg_count, 'social_list' => $return_array, 'filePath' => 'https://s3.amazonaws.com/trendyservice/post/');
        echo json_encode($result);
    }
    ...
    function asc_desc_recent($arr) {
        ...
        if ($flag == "P") {
            ...
            $lattitude2 = $lat_long['lat'];
            $longitude2 = $lat_long['long'];
            $result = array('status' => 'true', 'message' => $message, 'user_status' => $user_status, 'lat' => $lattitude2, 'longitude' => $longitude2, 'location' => $city, 'radius' => $radius, 'recent_posts' => $data, 'filePath' => 'https://s3.amazonaws.com/trendyservice/post/');
            echo json_encode($result);
        }
    }
    ...
    function trend_track($arr) {
        ...
        if (strcasecmp($arr['request_status'], "track") == 0) {
            if (strcasecmp($settings['profile'], "public") == 0) {
                ...
                $del_array = array("following" => $following_by, "followed_by" => $user_id);
                $this->users_model->del_prv_track($del_array);
...
//            echo $this->db->last_query();
    ...
    function notifications($arr) {
        ...
        $result = array('status' => 'true', 'message' => "success", 'notifications' => $notifications,'filePath_post' => 'https://s3.amazonaws.com/trendyservice/post/', 'filePath' => base_url() . "uploads/members/");
        echo json_encode($result);
    }
...
//        print
    ...
    function save_review($arr) {
        ...
        $review_id = $this->users_model->save_review($ins_array);
//        $flag = $this->users_model->check_log_reward($user_id, $post_id, "review");
//        if ($flag == 0) {
//            $point = 1;
//            $arr123 = array("user_id" => $user_id, "reward" => $point, "review_id" => $review_id, "reward_type" => "review", "product_id" => $post_id);
//            $this->users_model->update_rewards($arr123);
//        }
    ...
    function get_profile_details($arr) {
        ...
        $result = array('status' => 'true', 'reviewed_items' => $reviewd_items, 'tracking_count' => $tracking_count, 'saved_items' => $saved_items, 'user_detais' => $data, 'tracking_details' => $tracker, 'posted_items' => $posted_items, 'post_pic_path' => 'https://s3.amazonaws.com/trendyservice/post/', 'profile_pic_path' => "https://s3.amazonaws.com/trendyservice/members/", 'message' => 'user details');
        echo json_encode($result);
    }
    ...
    function page1_html() {
        $this->load->view("html/page1");
    }

    function page2_html() {
        $this->load->view("html/page2");
    }

    function pages($arr) {
        $page1 = base_url() . "client/page1_html";
        $page2 = base_url() . "client/page2_html";
        $result = array('status' => 'true', 'page1' => $page1, 'page2' => $page2, 'message' => 'success');
        echo json_encode($result);
    }
    ...
    function get_occasion_list($arr) {
        ...
        $result = array('status' => 'true', 'location' => $city, 'occasion_list' => $occasion, 'filePath' => "https://s3.amazonaws.com/trendyservice/occasion/", 'message' => 'success');
        echo json_encode($result);
    }
    ...
    function get_product_occasion($arr) {
        ...
        $result = array('status' => 'true', 'location' => $city, 'product_list' => $data, 'filePath' => 'https://s3.amazonaws.com/trendyservice/post/', 'message' => 'success');
        echo json_encode($result);
    }
    ...
    function product_details($arr) {
        ...
        $result = array('status' => 'true', 'radius' => $radius, 'product_details' => $details, 'suugested_items' => $suggested_items, 'filePath' => 'https://s3.amazonaws.com/trendyservice/post/');
        echo json_encode($result);
    }
...
//        $user_id = $arr['user_id'];
        ...
        $brandss = $this->users_model->get_brand_full($city, $gender, $data['type_array']);
        $max_min_PricePopularity = $this->users_model->pop_price_status($lat, $long, $city, $gender, $data['type_array'], $data['brand_array']);
        $result = array('status' => 'true', 'function_name' => 'save_filter', 'message' => "Succesfully updated", 'filter_data' => $data, "brand_list" => $brandss, 'min_max_price_popularity' => $max_min_PricePopularity, "type_list" => $category);
//        $result = array('status' => 'true','response'=>$g, 'message' => "Succesfully updated");
        echo json_encode($result);
    }
    ...
    function filter_occasion($arr) {
        ...
        $brands = $this->users_model->get_brand_full_occasion($city, $gender, $occasion_id, $data['type_array']);
        $max_min_PricePopularity = $this->users_model->pop_price_status_occasion($lat, $long, $gender, $user_id, $occasion_id);
        $result = array('status' => 'true', 'function_name' => 'save_filter', 'message' => "Succesfully updated", 'filter_data' => $data, "brand_list" => $brands, 'min_max_price_popularity' => $max_min_PricePopularity, "type_list" => $category);
...
//        $result = array('status' => 'true', 'message' => "Succesfully updated");
    ...
    function filter_trend($arr) {
        ...
        $brands = $this->users_model->get_brand_full_trend($city, $gender);
...
//        $brand = $this->users_model->get_brand_full($city, $gender);
        $category = $this->users_model->get_all_type($city, $gender);
        $brand = $this->users_model->get_brand_full($city, $gender, $data['type_array']);
        $max_min_PricePopularity = $this->users_model->pop_price_status($lat, $long, $city, $gender, $data['type_array'], $data['brand_array']);
...
//        $max_min_PricePopularity = $this->users_model->pop_price_status($lat, $long, $city, $gender);
    ...
    function get_filter_occasion($arr) {
        ...
        $asd = 0;
        $category = $this->users_model->get_all_type_occasion($city, $gender, $occasion_id);
        $brand = $this->users_model->get_brand_full_occasion($city, $gender, $occasion_id, $data['type_array']);
        $max_min_PricePopularity = $this->users_model->pop_price_status_occasion($lat, $long, $gender, $user_id, $occasion_id);
    ...
    function get_rewards($arr) {
        $user_id = $arr['user_id'];
        $rewards = $this->users_model->get_rewards($user_id);
        $result = array('status' => 'true', 'rewards' => $rewards, 'user_id' => $user_id);
        echo json_encode($result);
    }

    function filter_live_trend($arr) {
        $this->load->model('test_user');
        $fn_name = "filter_live_trend";
        $lat = $arr['lat'];
        $long = $arr['long'];
        $user_id = $arr['user_id'];
        $selected_type = $arr['sel_type'];
        $gender = $this->users_model->get_settings($user_id);
        $city = $this->users_model->find_Loc($lat, $long);
        $category_s = $this->users_model->get_all_type_trend($city, $gender);
        if ($selected_type == "category") {
            $category = $arr['category'];
            $brands = $arr['brand'];
            $brand = $this->users_model->get_brand_full_trend_cat($city, $gender, $category);
            $max_min_PricePopularity = $this->users_model->pop_price_status_trend_live($lat, $long, $city, $gender, $user_id, $category, $brands);
            $result = array('status' => 'true', 'function_name' => $fn_name, 'min_max_price_popularity' => $max_min_PricePopularity, "brand_list" => $brand, "type_list" => $category_s);
        } elseif ($selected_type == "brand") {
            $category = $arr['category'];
            $brands = $arr['brand'];
            $brand = $this->users_model->get_brand_full_trend_cat($city, $gender, $category);
            $max_min_PricePopularity = $this->users_model->pop_price_status_trend_live($lat, $long, $city, $gender, $user_id, $category, $brands);
            $result = array('status' => 'true', 'function_name' => $fn_name, 'min_max_price_popularity' => $max_min_PricePopularity, "brand_list" => $brand, "type_list" => $category_s);
        } elseif ($selected_type == "popularity") {
            $category = $arr['category'];
            $brands = $arr['brand'];
            $max_pop = $arr['pop_start'];
            $min_pop = $arr['pop_end'];
            $brand = $this->users_model->get_brand_full_trend_cat($city, $gender, $category);
            $max_min_PricePopularity = $this->users_model->pop_price_status_trend_live($lat, $long, $city, $gender, $user_id, $category, $brands, $max_pop, $min_pop);
            $result = array('status' => 'true', 'function_name' => $fn_name, 'min_max_price_popularity' => $max_min_PricePopularity, "brand_list" => $brand, "type_list" => $category_s);
        }
        echo json_encode($result);
    }
    ...
    function get_filter_trend($arr) {
        ...
        $max_min_PricePopularity = $this->users_model->pop_price_status_trend($lat, $long, $city, $gender, $user_id);
//        $category = $this->users_model->get_all_type_trend($city, $gender);
//        $brand = $this->users_model->get_brand_full_trend($city, $gender, $data['type_array']);
////        $max_min_PricePopularity = $this->users_model->pop_price_status_trend($lat, $long, $city, $gender, $user_id);
//        $max_min_PricePopularity = $this->users_model->pop_price_status_trend_live1($lat, $long, $city, $gender, $user_id, $data['type_array'], $data['brand_array']);
        $result = array('status' => 'true', 'filter_data' => $data, "brand_list" => $brand, 'min_max_price_popularity' => $max_min_PricePopularity, "type_list" => $category);
        echo json_encode($result);
    }
    ...
    function logout($arr) {
        ...
        $this->clear_filter_all(array('user_id' => $user_id));
    ...
    function profile($arr) {
        ...
        $saved_items = $this->users_model->get_user_saved_items($user_id);
        $posted_items = $this->users_model->get_posted_items($user_id);
        $result = array('status' => 'true', 'user_detais' => $data, 'tracking_details' => $tracker, 'reviewed_items' => $reviewd_items, 'saved_items' => $saved_items, 'posted_items' => $posted_items, 'post_pic_path' => 'https://s3.amazonaws.com/trendyservice/post/', 'profile_pic_path' => 'https://s3.amazonaws.com/trendyservice/members/', 'message' => 'user details');
        echo json_encode($result);
    }
...
//                        $image = file_get_contents($image_url); // sets $image to the contents of the url
                        ...
                        copy($image_url, $dest);
                        $targetFile = realpath("uploads/members") . "/" . $completefileName;
                        shell_exec("/usr/local/bin/s3cmd  --config=/var/www/html/trend/.s3cfg     --acl-public put " . $targetFile . "    s3://trendyservice/members/");
...
//            $userDetails = $this->users_model->getUserFullDetails(arr['user_id']);
    ...
    function Login($arr) {
        ...
        if ($msg == 'Login success') {
            ...
            $result = array('status' => 'true', 'message' => 'Login Successful', 'user' => $userdet, 'user_id' => $userdet['user_id'], 'email' => $userdet['user_email'], 'password' => $userdet['password'], 'profile_image' => $userdet['original_image'], 'name' => $userdet['name'], "profile_full_image" => $userdet['original_image'], "active" => $userdet['active']);
            //print_r($result);exit;
            echo json_encode($result);
        } else {
    ...
    function get_cms_datas() {
        $data_privacy = $this->users_model->get_cms_datas_privacy();
        $data_terms = $this->users_model->get_cms_datas_terms();
        $result = array('status' => 'true', "privacy" => $data_privacy['Answer'], 'terms' => $data_terms['Answer']);
        echo json_encode($result);
    }

    function get_gender($arr) {
        $user_id = $arr['user_id'];
        $gender = $this->users_model->get_settings($user_id);
        $result = array('status' => 'true', "gender" => $gender);
        echo json_encode($result);
    }

...
//        alert("SADSA");
        $.ajax({
            url: "<?php echo base_url() ?>admin/post/ajaxstatus",
    ...
    function toggleblock(id, block, blockdiv) {
        $.ajax({
            url: "<?php echo base_url() ?>admin/post/ajaxblock",
...
// function to delete masteraction 
        $("#deleteSel").click(function () {
            ...
            if (confirm('Are you sure to delete the selected occasio(s)?')) {
                $("#userMasterForm").attr("action", "<?php echo site_url("admin/post/index"); ?>");
...
// function to delete product 
        $("#bulkaction").change(function () {
            ...
            if (chkCnt == 0) {
                ...
                if (confirm('Are you sure to delete the selected user(s)?')) {
                    $("#userMasterForm").attr("action", "<?php echo site_url("admin/post/bulkAction"); ?>");
                    $("#userMasterForm").submit();
                    return true;
                }
                else {
        ...
        $('.sortlink').click(
                function () {
                    ...
                    $("#userMasterForm").attr("action", "<?php echo site_url("admin/post/lists"); ?>");
                    $("#userMasterForm").submit();
                    return true;
                });
...
<form name="userMasterForm" id="userMasterForm" method="post" action="<?php echo base_url(); ?>admin/post/lists">
    ...
    <div class="row" id="Title">
        <div class="col-lg-12"><legend>Post List</legend></div>
    </div>
    ...
    <div class="row" <?php if (!$message && !$error) { ?>style="display:none" <?php } ?> id="msg_head">
        <?php if ($message) { ?>
            ...
            <div class="alert alert-danger col-lg-12 col-offset-1"><?php echo $error; ?>
                <!--danger-info-->
                <button data-dismiss="alert" class="close" type="button">x</button>
            </div><?php } ?>
    </div>
    <div class="col-offset-1 col-lg-3">
        <div class="input-group">
            ...
            <select name="gender" id="sta" onchange="filter_gender()" class="form-control" >
                <option <?php if ($gender == '') { ?> selected <?php } ?> value="" <?php if ($status == '') { ?> selected="selected"<?php } ?> >Gender</option>
    ...
    <div class="row form-group">
        ...
        <div class="col-lg-6">
            ...
            <div class="input-group">
                ...
                <span class="input-group-btn btn_search">
                    <a href="javascript:" onclick="window.location.href = '<?php echo base_url() ?>admin/post/index'" class="blu_btn">
    ...
    <table class="table table-bordered table-hover" > 
        <thead>
            ...
            <tr>
                ...
                <th width="50"><center>Brand</center></th>
        <th width="60"><center>Description</center></th>
        <th width="50"><center>Occasion Name</center></th>
        <th width="50"><center>Category Name</center></th>
        <th width="40"> <center>Price($)</center></th>
        <th width="40"><center> Gender</center></th>
        <th width="40"> <center>Image</center></th>
        <th width="70">        <center> Link</center>        </th>
        <th width="40"><center>Created By</center></th>
        <th width="40"><center>            Created on</center>        </th>
            ...
            <?php if (sizeof($userlist) > 0) : //echo "<pre>"; print_r($userlist); exit;  ?>
                <?php foreach ($userlist as $val) : ?>
                    ...
                    <tr>
                        ...
                        <td width="5"><center><a href="<?php echo base_url(); ?>admin/post/delete/<?php echo $val->id; ?>?limit=<?php echo $limit; ?>&per_page=<?php echo $per_page; ?>" onclick="return confirm('Are you sure you want to delete?');"><i class="glyphicon glyphicon-trash" title="Delete"></i></a></center></td>


                <td width="5"><center>

                    <a href="<?php echo base_url(); ?>admin/post/details/<?php echo $val->id; ?>" class="link1">
                        <i class="glyphicon glyphicon-eye-open" title="View Details"></i></a></center>
                </td>
                <td width="5"><center>

                    <a href="<?php echo base_url(); ?>admin/post/editdetails/<?php echo $val->id; ?>" class="link1">
                        <i class="glyphicon glyphicon-edit" title="Edit Details"></i></a></center>
                </td>
                <td><?= $val->brand; ?></td>
                <td width="100"><?= $val->description; ?></td>
                <td><?= $val->occasion_name; ?></td>
                <td><?= $val->category_name; ?></td>
                <td><?= $val->price; ?></td>
                <td><?= $val->gender; ?></td>
                <td style="max-width: 130px;"><img class="img-responsive" src="https://s3.amazonaws.com/trendyservice/post/<?= $val->fileNAME; ?>" height="140" style="max-width: 100px !important" ></td>
                <td style="max-width: 130px;word-wrap: break-word;">
                    <a href="<?= $val->product_url ?>" target="_blank">
                        <?= $val->product_url ?></a>
                </td>
                <td><?= $val->user_name; ?></td>
                <td><?php echo date(getConfigValue('date_format'), strtotime($val->created_on)); ?>
                </td>
...
<script>
    ...
    function view_preferevce(post_id)
    {
        $("#myModal").html("");
        $.ajax({
            type: "post",
    ...
    function view_feedback(post_id)
    {
        $("#myModal").html("");
        $.ajax({
            type: "post",
            url: "<?php echo base_url(); ?>admin/post/feedback",
            data: {'post_id': post_id},
            success: function (data) {
                $("#myModal").html(data);
                //alert(data);

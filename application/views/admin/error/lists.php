<script language="javascript">
    ...
    function toggleStatus(id, status, statusdiv) {
        $.ajax({
            url: "<?php echo base_url() ?>admin/report/ajaxstatus",
    ...
    function toggleblock(id, block, blockdiv) {
        $.ajax({
            url: "<?php echo base_url() ?>admin/report/ajaxblock",
...
// function to delete masteraction 
        $("#deleteSel").click(function () {
            ...
            if (confirm('Are you sure to delete the selected occasio(s)?')) {
                $("#userMasterForm").attr("action", "<?php echo site_url("admin/report/index"); ?>");
...
// function to delete product 
        $("#bulkaction").change(function () {
            ...
            if (chkCnt == 0) {
                ...
                if (confirm('Are you sure to delete the selected user(s)?')) {
                    $("#userMasterForm").attr("action", "<?php echo site_url("admin/report/bulkAction"); ?>");
                    $("#userMasterForm").submit();
                    return true;
                }
                else {
        ...
        $('.sortlink').click(
                function () {
                    ...
                    $("#userMasterForm").attr("action", "<?php echo site_url("admin/report/lists"); ?>");
                    $("#userMasterForm").submit();
                    return true;
                });
        ...
        $(document).on('change', '#limit', function () {
            $("#userMasterForm").attr("action", "<?php echo site_url("admin/error"); ?>");
            $("#userMasterForm").submit();
            return true;
        });
...
<form name="userMasterForm" id="userMasterForm" method="post" action="<?php echo base_url(); ?>admin/error">
    <input type="hidden" name="order_by_field" id="order_by_field" />
    ...
    <div class="row" id="Title">
        <div class="col-lg-12"><legend>Error Lists</legend></div>
    </div>
    ...
    <div class="row form-group">
        <div class="col-lg-6">
            ...
            <div class="input-group">
                ...
                <span class="input-group-btn btn_search">
                    <a href="javascript:" onclick="window.location.href = '<?php echo base_url() ?>admin/error/index'" class="blu_btn">
            ...
            <?php if (sizeof($userlist) > 0) : ?>
                <?php foreach ($userlist as $val) : ?>
                    ...
                    <tr>
                        ...
                        <td width="5"><center><a href="<?php echo base_url(); ?>admin/error/delete/<?php echo $val->id; ?>?limit=<?php echo $limit; ?>&per_page=<?php echo $per_page; ?>" onclick="return confirm('Are you sure you want to delete?');"><i class="glyphicon glyphicon-trash" title="Delete"></i></a></center></td>



    <td style="max-width: 170px;word-wrap: break-word;"><a href="<?= $val->link; ?>" target="_blank"><?= $val->link; ?></a></td>
                <td><?php
                    if ($val->posted_by != '') {
                        echo $val->posted_by;
                    } else {
                        echo "This user account was deleted";
                    }
                    ?>  </td>

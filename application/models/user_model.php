...
class User_model extends MY_Model {
    ...
    function update_user_ext($arr1, $arr2) {
        return $this->db->update("user_master", $arr1, $arr2);
    }

    function get_bot_loc($loc) {
        $location = $this->db->query("select * from trendy_city where id='$loc'");
    ...
    function del_brand($arr, $array, $id) {

        $post = $this->db->query("select id from trend_post where brand='$id'");
        ...
        foreach ($result as $res) {
            ...
            $this->db->delete("post_image", array("id" => $post_id));
            $this->db->delete("trend_review", array("post_id" => $post_id));
            $this->db->delete("trendy_notifications", array("post_id" => $post_id));
            $this->db->delete("report_master", array("product_id" => $post_id));
        }
        $this->db->delete("trend_post", $array);
        return $this->db->delete("brand_master", $arr);
    }
    ...
    function update_brand($arr, $con) {
        return $this->db->update("brand_master", $arr, $con);
    }
    ...
    function get_brand_details($id) {
        $data = $this->db->query("select * from brand_master where id='$id'");
//        echo $this->db->last_query();
    ...
    public function getUserLists($status = '', $limit, $start = 0, $key) {
        ...
        $start = $start ? $start : 0;
        $sql = "SELECT user_master.* FROM user_master where 1";
        if ($status)
            $sql .= " AND user_master.active='$status' ";

        if ($key != '') {
            $sql .= " AND (user_master.user_email LIKE '%$key%' or user_master.name LIKE '%$key%')";
        }
        $sql .= " GROUP BY user_master.user_id";
        $sql .= " ORDER BY user_master.user_id DESC";
        if ($limit)
            $sql .= " LIMIT $start, $limit";
        $result = $this->db->query($sql);
        $results = $result->result();
...
//exit;
    ...
    function save_filter_cat($cat_id, $parent_id) {
        $data = $this->db->query("select user_id from filter_type_child where type_id='$parent_id' ");
        $category = $data->result_array();
        foreach ($category as $cat) {
            $this->db->insert("filter_type_child", array("type_id" => $cat_id, "user_id" => $cat['user_id']));
        }
        return TRUE;
    }
...
//        exit;
 ...
 function get_parent_gender($gender) {
        $data = $this->db->query("select * from category where gender='$gender'");
        $parent = $data->result_array();
        $return_array = array();
        $i = 0;
        foreach ($parent as $key => $par) {
...
//            echo "ist ".$parent_id."<br/>";
            if ($parent_id == 0 || $parent_id == '0') {
                ...
                $p1 = $this->db->query("select parent_id from category where category_id='$parent_id' and gender='$gender'");
//                echo "select parent_id from category where category_id='$parent_id'";
                $p2 = $p1->row_array();
//                print_r($p2);
                if ($p2['parent_id'] == '0' || $p2['parent_id'] == 0) {
...
//        exit;
    ...
    function change_cat_gender($gender) {
        $qry = $this->db->query("select * from category where gender='$gender' order by name asc");
        return $qry->result_array();
    }

    function get_all_data_table($table, $contn_table = '', $contn = '') {

        $sql = "select * from " . $table . " where 1 = 1 ";
        if ($contn_table && $contn) {
            $sql .= "and " . $contn_table . "='$contn'";
        }
    ...
    function get_lat_long($name) {
        $data = $this->db->query("SELECT * FROM trendy_city WHERE city='$name'");
        return $data->row_array();
    }

    function inser_post_values($data) {
        $this->db->insert("trend_post", $data);
        return $this->db->insert_id();
    }

    function inser_post_image($data) {

...
//create a new bucket
        ...
        $msg3 = $s3->putObjectFile($temp_name, $bucket_name, $foldername, S3::ACL_PUBLIC_READ);
...
//           echo $this->db->last_query()."<br/>";
                ...
                $count3 = $this->db->query("select count(*) as count3 from trend_post where product_type='$cat_id' and posted_locatn='$city' and gender='$gender' and occasion_id='$occasion_id'");
//                    echo $this->db->last_query()."<br/>";
//                    exit;
                $coun3 = $count3->row_array();
//                  echo $this->db->last_query() . "<br/>";
    ...
    function del_prv_track($arr) {
        return $this->db->delete("tracking_user", $arr);
    }
...
//            echo sizeof($parent_array);
    ...
    function get_userStatus($id) {
        $data = $this->db->query("select active,active_flag from user_master where user_id='$id'");
        $return = $data->row_array();
        if ($return['active_flag'] == "Y" && $return['active'] == "Y") {
            $status = "Y";
        } else if ($return['active_flag'] == "N" || $return['active'] == "N") {
            if ($return['active_flag'] == "N") {
                $status = "N";
            } else {
                $status = "B";
            }
    ...
    function get_image_array($post_id) {
        $data = $this->db->query("select image_name from post_image where post_id='$post_id' ORDER BY id ASC");
        return $data->result_array();
    }
...
//        print_r($array4);
    ...
    function get_recent_post3($gender, $latti1, $longitude1, $user_id) {
        $city = $this->find_Loc($latti1, $longitude1);
        $lat_long = $this->get_bot_loc($city);
        $lattitude = $lat_long['lat'];
        $longitude = $lat_long['long'];
...
# here we do a little extra for longitude
    ...
    function getImage($id) {
        $data = $this->db->query("select fileNAME from trend_post where id=$id");
        return $data->row_array();
    }

    function get_myNotifications($user_id) {
        $note = $this->db->query("select t1.*, t2.name as user_name ,t2.img_extension, t3.name as from_userName , t4.product_name as product_name,t4.fileNAME as post_image from trendy_notifications t1 left join user_master t2 on t1.to_user_id=t2.user_id left join user_master t3 on t1.from_user_id = t3. user_id left join trend_post t4 on t4.id=t1.post_id where t1.to_user_id='$user_id' order by t1.id desc");
    ...
    function trend_vote($arr) {
        ...
        if ($vote_count1 == 5 || $vote_count1 == 10 || $vote_count1 == 20 || (($vote_count1 % 50 ) == 0 && $vote_count1 != 0)) {
        ...
        if ($vote_count == 10 || $vote_count == 25 || $vote_count == 50 || $vote_count == 100) {
            if ($vote_count == 10) {
                $point = 5;
            } elseif ($vote_count == 25) {
                $point = 10;
            } elseif ($vote_count == 50) {
                $point = 20;
            } elseif ($vote_count == 100) {
                $point = 30;
            }
            $chk_vote_flag = $this->db->query("select count(*) as count from reward_master where net_vote=$vote_count and product_id='$post_id' and user_id='$post_user_id' and reward_type='net_vote'");
            $cooo = $chk_vote_flag->row_array();
//            echo $this->db->last_query();
            if ($cooo['count'] < 1) {
                $in_arr = array("reward" => $point, "user_id" => $post_user_id, "net_vote" => $vote_count, "product_id" => $post_id, "reward_type" => "net_vote");
                $this->db->insert("reward_master", $in_arr);
//                echo $this->db->last_query();
            }
        } elseif ($vote_count == 11 || $vote_count == 26 || $vote_count == 51 || $vote_count == 101) {
            $net_vote_c = $vote_count - 1;
            if ($net_vote_c == 10) {
                $point = 5;
            } elseif ($net_vote_c == 25) {
                $point = 10;
            } elseif ($net_vote_c == 50) {
                $point = 20;
            } elseif ($net_vote_c == 100) {
                $point = 30;
            }
            $chk_vote_flag = $this->db->query("select count(*) as count from reward_master where net_vote='$net_vote_c' and product_id='$post_id' and user_id='$post_user_id' and reward_type='net_vote'");
            $cooo = $chk_vote_flag->row_array();
            if ($cooo['count'] < 1) {
                $in_arr = array("reward" => $point, "user_id" => $post_user_id, "net_vote" => $net_vote_c, "product_id" => $post_id, "reward_type" => "net_vote");
                $this->db->insert("reward_master", $in_arr);
            }
        } elseif ($vote_count == 9 || $vote_count == 24 || $vote_count == 49 || $vote_count == 99) {
            $net_vote_c = $vote_count + 1;
            if ($net_vote_c == 10) {
                $point = 5;
            } elseif ($net_vote_c == 25) {
            ...
            if ($cooo['count'] > 0) {
                $in_arr = array("reward" => $point, "user_id" => $post_user_id, "net_vote" => $net_vote_c, "product_id" => $post_id, "reward_type" => "net_vote");
                $this->db->delete("reward_master", $in_arr);
            }
        } elseif ($vote_count == 8 || $vote_count == 23 || $vote_count == 48 || $vote_count == 98) {
            $net_vote_c = $vote_count + 2;
            if ($net_vote_c == 10) {
                $point = 5;
            } elseif ($net_vote_c == 25) {
                $point = 10;
        ...
        $point = 1;
        $chek_cast_vote_flag = $this->db->query("select count(*) as count from reward_master where   product_id='$post_id' and user_id='$user_id' and reward_type='vote' and reward='$point'");
        $cast_count = $chek_cast_vote_flag->row_array();
        if ($cast_count['count'] == 0) {
            $vote_Array = array("product_id" => $post_id, "reward_type" => "vote", "user_id" => $user_id, "reward" => $point);

            $this->db->insert("reward_master", $vote_Array);
        }
...
//        var_dump($result);
    ...
    function get_social($id, $latitude, $longitude) {
        $user_gender = $this->get_settings($id);
        $data = $this->db->query("select T1.*,T2.name as user_name,T3.product_name,T3.id as post_id,T3.fileNAME,T3.lat,T3.long,T3.posted_locatn  from tracking_user T1 left join user_master T2 on T1.following=T2.user_id left join trend_post T3 on T3.user_id=T1.following where T1.followed_by='$id' and T3.id is not null and T3.gender='$user_gender' and T1.request_status='accept' group by T3.id order by T3.id desc");
...
//            foreach ($udata as $key2 => $uda) {
            ...
            $upv = $up->row_array();
            $user_n_arr = explode(" ", $soc['user_name']);
            $user_name1 = "";
            foreach ($user_n_arr as $key098 => $ar_us) {
                $user_name1.=$ar_us[0];
            }
            $social[$key]['user_name1'] = $user_name1;
            $up_vote = $upv['upcount'];
...
//            $ret_array[] = $social;
    ...
    function check_login($user, $pwd) {
        $qry = $this->db->query("select *,count(*) as count  from user_master where (user_email='$user' OR username='$user') and password='" . md5($pwd) . "'");
        return $qry->row_array();
    }

    function check_varification($user_id, $code) {
        $qry = $this->db->query("select count(*) as count  from user_master where user_id='$user_id' and activation_code='$code'");
        $count = $qry->row_array();
        if ($count['count'] == 1) {
            $this->db->query("UPDATE user_master set active_flag='Y' where user_id='$user_id'");
            return 1;
        } else {
            return 0;
        }
    ...
    function get_user_email($email) {
        $data = $this->db->query("select * from user_master where user_email='$email'");
        $user = $data->row_array();
        if (sizeof($user) > 0) {
            return $user;
        } else {
    ...
    function Email_varification() {
        $data = $this->db->query("select * from email_config where email_title='Email_varification'");
        $email = $data->row_array();
        return $email['email_template'];
    }

    function fgt_pwd_update($arr, $cond) {
        $this->db->update("user_master", $arr, $cond);
//        echo $this->db->last_query();
        return true;
    }

    function get_fgt_pwd() {
        $data = $this->db->query("select * from email_config where email_title='Forgot_password'");
        $email = $data->row_array();
        return $email['email_template'];
    }
    ...
    function get_brand_full($city, $gender, $arr) {
        $query = "SELECT * FROM (select T1.*,(SELECT count(*) AS num  FROM trend_post WHERE brand=T1.id and posted_locatn='$city' and gender='$gender' ";
        $flag = 0;
        foreach ($arr as $array) {
            if ($flag == 0) {
                $query.=" AND ( product_type='" . $array['type_id'] . "' ";
                $flag = 1;
            } else {
                $query.=" or  product_type='" . $array['type_id'] . "' ";
            }
        }if ($flag == 1) {
            $query.=" ) ";
        }
        $query.=" ) AS count_num from brand_master T1 left join trend_post T2 on T1.id=T2.brand  GROUP BY T1.brand order by T1.brand asc) as brand_array WHERE count_num!=0";
        $data = $this->db->query($query);
        return $data->result_array();
    }

    function get_brand_full_trend($city, $gender, $arr) {
        $query = "SELECT * FROM (select T1.*,(SELECT count(*) AS num  FROM trend_post WHERE brand=T1.id and posted_locatn='$city' and gender='$gender'";
        $cat_flags = 0;
        foreach ($arr as $array_cat) {
            if ($cat_flags == 0) {
                $cat_flags = 1;
                $query.=" AND ( product_type='" . $array_cat['type_id'] . "'";
            } else {
                $query.=" OR product_type='" . $array_cat['type_id'] . "'";
            }
        }
        if ($cat_flags != 0) {
            $query.=" ) ";
...
//           echo $this->db->last_query();
    ...
    function get_brand_full_trend_cat($city, $gender, $cat) {
        ...
        foreach ($explode as $exp) {
            if ($cy == 0) {
                $query.="AND ( product_type='$exp' ";
                $cy++;
            } else {
                $query.=" or product_type=' $exp'";
            }
        }
        if ($cy != 0) {
            $query.= ")";
...
//        print_r($ret_array);
    ...
    function get_brand_full_occasion($city, $gender, $occasion_id, $arr) {
        $qry = "SELECT * FROM (select T1.*,(SELECT count(*) AS num  FROM trend_post WHERE brand=T1.id and posted_locatn='$city' and gender='$gender' and occasion_id='" . $occasion_id . "'";
        $flag = 0;
        foreach ($arr as $array) {
            if ($flag == 0) {
                $qry.=" AND ( product_type='" . $array['type_id'] . "' ";
                $flag = 1;
            } else {
                $qry.=" or  product_type='" . $array['type_id'] . "' ";
            }
        }if ($flag == 1) {
            $qry.=" ) ";
        }
        $qry.= ") AS count_num from brand_master T1 left join trend_post T2 on T1.id=T2.brand  GROUP BY T1.brand order by T1.brand asc) as brand_array WHERE count_num!=0";
        $data = $this->db->query($qry);
        $result = $data->result_array();
...
//        return $return_array;
    ...
    function pop_price_status($lat, $long, $city, $gender, $category, $brand) {
        $qry1 = "select price from trend_post where posted_locatn='$city' and gender='$gender'";
        $qy1f = 0;
        foreach ($category as $cat) {
            if ($qy1f == 0) {
                $qry1.=" And (product_type='" . $cat['type_id'] . "' ";
                $qy1f = 1;
            } else {
                $qry1.=" or product_type='" . $cat['type_id'] . "' ";
            }
        }if ($qy1f != 0) {
            $qry1.= " ) ";
        }
        $qy1fb = 0;
        foreach ($brand as $rands) {
            if ($qy1fb == 0) {
                $qry1.=" And (brand='" . $rands['brand_id'] . "' ";
                $qy1fb = 1;
            } else {
                $qry1.=" or brand='" . $rands['brand_id'] . "' ";
            }
        }if ($qy1fb != 0) {
            $qry1.= " ) ";
        }
        $qry1.=" order by price desc limit 1";
//        echo $qry1;
//        exit;
        $price_max = $this->db->query($qry1);
        $primax = $price_max->row_array();
        $return['max_price'] = $primax['price'];
        $qry2 = "select price from trend_post where posted_locatn='$city' and gender='$gender'";
        $qy1f1 = 0;
        foreach ($category as $cat) {
            if ($qy1f1 == 0) {
                $qry2.=" And (product_type='" . $cat['type_id'] . "' ";
                $qy1f1 = 1;
            } else {
                $qry2.=" or product_type='" . $cat['type_id'] . "' ";
            }
        }if ($qy1f1 != 0) {
            $qry2.= " ) ";
        }
        ...
        foreach ($brand as $rands) {
            if ($qy1fb1 == 0) {
                $qry2.=" And (brand='" . $rands['brand_id'] . "' ";
                $qy1fb1 = 1;
            } else {
                $qry2.=" or brand='" . $rands['brand_id'] . "' ";
            }
        }if ($qy1fb1 != 0) {
            $qry2.= " ) ";
        }
        $qry2.=" order by price asc limit 1";
        $price_max1 = $this->db->query($qry2);
        $primax1 = $price_max1->row_array();
        $return['min_price'] = $primax1['price'];
        $qry3 = "SELECT *, (SELECT COUNT(vote) FROM trend_vote WHERE vote='up' AND post_id=T2.post_id ) -(SELECT COUNT(vote) FROM trend_vote WHERE vote='down' AND post_id=T2.post_id ) as maxCountss
FROM (SELECT id as id1 FROM trend_post WHERE posted_locatn='$city' and gender='$gender'";
        $qy1f3 = 0;
        foreach ($category as $cat) {
            if ($qy1f3 == 0) {
                $qry3.=" And (product_type='" . $cat['type_id'] . "' ";
                $qy1f3 = 1;
            } else {
                $qry3.=" or product_type='" . $cat['type_id'] . "' ";
            }
        }if ($qy1f3 != 0) {
            $qry3 = $qry3 . " ) ";
        }
        ...
        foreach ($brand as $rands) {
            if ($qy1fb3 == 0) {
                $qry3.=" And (brand='" . $rands['brand_id'] . "' ";
                $qy1fb3 = 1;
            } else {
                $qry3.=" or brand='" . $rands['brand_id'] . "' ";
            }
        }if ($qy1fb3 != 0) {
            $qry3.= " ) ";
        }
        $qry3.= ") as T1 LEFT JOIN trend_vote T2 ON T1.id1=T2.post_id GROUP BY T2.post_id ORDER BY maxCountss asc limit 1";
        $pop_mi = $this->db->query($qry3);
        ...
        $qry4 = "SELECT *, (SELECT COUNT(vote) FROM trend_vote WHERE vote='up' AND post_id=T2.post_id ) -(SELECT COUNT(vote) FROM trend_vote WHERE vote='down' AND post_id=T2.post_id ) as maxCountss
FROM (SELECT id as id1 FROM trend_post WHERE posted_locatn='$city' and gender='$gender'";
        $qy1f4 = 0;
        foreach ($category as $cat) {
            if ($qy1f4 == 0) {
                $qry4.=" And (product_type='" . $cat['type_id'] . "' ";
                $qy1f4 = 1;
            } else {
                $qry4.=" or product_type='" . $cat['type_id'] . "' ";
            }
        }if ($qy1f4 != 0) {
            $qry4 = $qry4 . " ) ";
        }
        ...
        foreach ($brand as $rands) {
            if ($qy1fb4 == 0) {
                $qry4.=" And (brand='" . $rands['brand_id'] . "' ";
                $qy1fb4 = 1;
            } else {
                $qry4.=" or brand='" . $rands['brand_id'] . "' ";
            }
        }if ($qy1fb4 != 0) {
            $qry4.= " ) ";
        }
        $qry4.= " ) as T1 LEFT JOIN trend_vote T2 ON T1.id1=T2.post_id GROUP BY T2.post_id ORDER BY maxCountss desc limit 1";
        $pop = $this->db->query($qry4);
...
//        return $return_array;
    ...
    function pop_price_status_trend_live($latti1, $longitude1, $city, $gender, $user_id, $category, $brands, $max_pop = '', $min_pop = '') {
        $city = $this->find_Loc($latti1, $longitude1);
        $lat_long = $this->get_bot_loc($city);
        $lattitude = $lat_long['lat'];
        $longitude = $lat_long['long'];
...
//$radius = $key  = $this->get_google_search_radius(); 
        ...
        if ($gender != 'both') {
            $qry = "SELECT min(price) as min_price,max(price) as max_price,min(vote_count) as min_vote,max(vote_count) as max_vote FROM (select test.*,price,fileName,( 3959 * acos( cos( radians($lattitude) ) * cos( radians( lat ) ) * cos( radians(`long`) - radians($longitude) ) + sin( radians($lattitude) ) * sin( radians( lat ) ) ) ) AS distance   from (SELECT trend_post.id,((SELECT COUNT(*) FROM trend_vote WHERE trend_post.id=post_id AND vote='up')  - 
(SELECT COUNT(*) FROM trend_vote WHERE trend_post.id=post_id AND vote='down') ) as vote_count FROM trend_post  ";
            $cy = 0;
            foreach ($explode as $exp) {
                if ($cy == 0) {
                    $qry.="where ( product_type='$exp' ";
                    $cy++;
                } else {
                    $qry.=" or product_type=' $exp'";
                }
            }
            if ($cy != 0) {
                $qry.= ")";
            ...
            $br1 = 0;
            foreach ($brand_array as $brand1) {

                if ($br1 == 0) {
                    if ($cy == 0) {
                        $qry.=" where ( brand='$brand1' ";
                    } else {
                        $qry.=" AND ( brand='$brand1' ";
                    }
                    $br1++;
                } else {
                    $qry.=" or brand=' $brand1'";
            ...
            $qry.=") AS test LEFT JOIN trend_post ON trend_post.id=test.id  where gender='" . $gender . "' and vote_count >=20";
        } else {
            $qry = "SELECT min(price) as min_price,max(price) as max_price,min(vote_count) as min_vote,max(vote_count) as max_vote FROM (select test.*,price,fileName,( 3959 * acos( cos( radians($lattitude) ) * cos( radians( lat ) ) * cos( radians(`long`) - radians($longitude) ) + sin( radians($lattitude) ) * sin( radians( lat ) ) ) ) AS distance   from (SELECT trend_post.id,((SELECT COUNT(*) FROM trend_vote WHERE trend_post.id=post_id AND vote='up')  - 
(SELECT COUNT(*) FROM trend_vote WHERE trend_post.id=post_id AND vote='down') ) as vote_count FROM trend_post";
            $cy = 0;
            ...
            $qry.=") AS test LEFT JOIN trend_post ON trend_post.id=test.id  where";
            if ($max_pop != '') {
                $qry.="vote_count >='" . $min_pop . "' and vote_count <='" . $max_pop . "' ";
            } else {
...
//        return $return_array;
    ...
    function pop_price_status_trend_live1($latti1, $longitude1, $city, $gender, $user_id, $category, $brands, $max_pop = '', $min_pop = '') {
        $city = $this->find_Loc($latti1, $longitude1);
        $lat_long = $this->get_bot_loc($city);
        $lattitude = $lat_long['lat'];
        $longitude = $lat_long['long'];
...
//        return $return_array;
    ...
    function get_rewards($user_id) {
        $dAZ = $this->db->query("SELECT SUM(reward) as reward FROM reward_master where user_id='$user_id'");
        $re = $dAZ->row_array();
        if ($re['reward'] == '') {
            $re['reward'] = 0;
        }
    ...
    function update_rewards($arr) {
        $this->db->insert("reward_master", $arr);
        return TRUE;
    }

    function update_reward_pdt($array, $condition) {
        $this->db->update("trend_post", $array, $condition);
        return TRUE;
    }

    function up_reward_review($array, $arr, $condition) {
        $this->db->update("trend_review", $arr, $condition);
        return $this->db->insert("reward_master", $array);
    }

    function get_reward_data() {
        $sql = "SELECT id,user_id,created_on FROM trend_post WHERE DATE(created_on) <= DATE_SUB(CURDATE(), INTERVAL 5 DAY) and reward_flag='N'";
        $data = $this->db->query($sql);
        return $data->result_array();
    }

    function get_review_reward_data() {
        $sql = "SELECT * FROM trend_review WHERE DATE(reviewed_on) <= DATE_SUB(CURDATE(), INTERVAL 2 DAY)  and updated_on is null";
        $data = $this->db->query($sql);
        return $data->result_array();
    }

    function check_log_reward($user_id, $pdt_id, $type) {
        $count = $this->db->query("select count(*) as count from reward_master where user_id='$user_id' and product_id='$pdt_id' and reward_type='$type'");
        $cou = $count->row_array();
        return $cou['count'];
    }
...
# function to get admin email address
    ...
    public function get_cms_datas_privacy() {
        $sql = "SELECT Answer FROM cms_master WHERE `status`='Y' AND Question='Privacy'";
        $query = $this->db->query($sql);
        return $query->row_array();
    }

    public function get_cms_datas_terms() {
        $sql = "SELECT Answer FROM cms_master WHERE `status`='Y' AND Question='Terms'";
        $query = $this->db->query($sql);
        return $query->row_array();
    }
